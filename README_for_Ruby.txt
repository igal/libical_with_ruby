...
I'd be grateful if someone could provide me with a trivial C program that uses
libical to read a string of iCalendar data, returns an array of events, and
then iterates over these to print the contents of these data structures.

Either the C code is buggy or too confusing, but I can't figure out how to do
the above task. I've tried three different ways, but they all return nulls
instead of the promised data structures.
...
- Review "src/libical/icalparser.c", which does the parsing and its code is the
  only real documentation

- Review "examples/parse_text_main.c", my trivial example that parses a file
  and displays its contents. However, I cannot access the data structures. When
  writing your code, please review this file AND the "run" task in
  "examples/Rakefile", because you really don't want to waste time trying to
  setup the environment yourself because I already have. To compile and run the
  C example:
...
- To rerun the examples after you've made a change to C code, you'll need to
  rerun one of the two commands above but should leave out the "./configure"
  step.
